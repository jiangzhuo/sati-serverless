...
export class NatureAlbumService {
    ...
    async getNatureAlbumRecord(userId, natureAlbumId) {
        if (isArray(natureAlbumId)) {
            ...
            return await this.natureAlbumRecordModel.findOne({ userId, natureAlbumId }).exec();
        }
    }
    ...
    async updateTag(id) {
        const doc = await this.natureAlbumModel.findOne({ _id: id }).exec();
        ...
        await this.natureAlbumModel.updateOne({ _id: id }, { __tag: ['*'].concat(nameCut).concat(descriptionCut).concat(copyCut) }).exec();
    }
    ...
    async updateNatureAlbum(id, data) {
        ...
        const result = await this.natureAlbumModel.findOneAndUpdate({ _id: id }, updateObject, { new: true }).exec();
        await this.updateTag(result._id);
        return result;
    }
    ...
    async favoriteNatureAlbum(userId, natureAlbumId) {
        const result = await this.natureAlbumRecordModel.findOneAndUpdate({
            userId,
            natureAlbumId,
    ...
    async finishNatureAlbum(userId, natureAlbumId, duration) {
        const currentRecord = await this.natureAlbumRecordModel.findOne({ userId, natureAlbumId });
    ...
    async buyNatureAlbum(userId, natureAlbumId, discount) {
        ...
        const finalPrice = Math.floor(natureAlbum.price * discountVal / 100);
        ...
        try {
            ...
            await this.accountModel.create([{
                ...
                extraInfo: JSON.stringify({ resource: natureAlbum, discount }),
            }], { session });
            const natureAlbumRecord = await this.natureAlbumRecordModel.findOneAndUpdate(
                { userId, natureAlbumId },
    ...
    async searchNatureAlbum(keyword, from, size) {
        ...
        const total = await this.natureAlbumModel.countDocuments(query);
        const data = await this.natureAlbumModel.find(query).skip(from).limit(size).exec();
        return { total, data };
    }
}
